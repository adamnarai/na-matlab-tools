...
if length(varargin) < 5
    handles.X_var_name = 'X';
else
    handles.X_var_name = varargin{5};
end

if length(varargin) < 6
    handles.study_name = '';
else
    handles.study_name = varargin{6};
end
...
data = handles.ET_results.data(1, handles.stim_idx, 1).(handles.X_var_name) - 199;
data_fix = data;
...
data_gliss = data;
data_gliss(~logical(handles.ET_results.glissadeIdx(handles.stim_idx).Idx)) = NaN;
...
plot(ax, data_gliss, time, 'g');
hold(ax, 'off');
xlim(ax, [0 img_size]);
xlabel(ax, 'X position [px]');
ylabel(ax, 'Time [s]');
legend({'other','fix','sacc','gliss'}, 'location', 'southeast');
...
set(handles.title, 'string', [handles.study_name]);
